...
export class Table implements OnInit, AfterViewInit, AfterContentInit, BlockableUI {
  ...
  selectRange(event: MouseEvent, rowIndex: number) {
    ...
    for (let i = rangeStart; i <= rangeEnd; i++) {
      ...
      if (!this.isSelected(rangeRowData)) {
        ...
        const dataKeyValue: string | null = this.dataKey ? String(ObjectUtils.resolveFieldData(rangeRowData, this.dataKey)) : null;
        if (dataKeyValue) {
          this.selectionKeys[dataKeyValue] = 1;
        }
  ...
  onColumnResizeEnd(event, column) {
    ...
    const minWidth = parseInt(column.style.minWidth || 15);
  ...
  setScrollableItemsWidthOnExpandResize(column, newColumnWidth, delta) {
    const scrollableView = column ? this.findParentScrollableView(column) : this.containerViewChild.nativeElement;
    ...
    const scrollableBodyTable = DomHandler.findSingle(scrollableBody, 'table.ui-table-scrollable-body-table');
...
export class ScrollableView implements AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  ngAfterViewInit() {
    if (!this.frozen) {
      ...
      const scrollableView = this.el.nativeElement.nextElementSibling;
      if (scrollableView) {
  ...
  setScrollHeight() {
    if (this.scrollHeight && this.scrollBodyViewChild && this.scrollBodyViewChild.nativeElement) {
      if (this.scrollHeight.indexOf('%') !== -1) {
        ...
        const containerHeight = DomHandler.getOuterHeight(this.dt.el.nativeElement.children[0]);

        if (this.scrollHeight.includes('calc')) {
          const percentHeight = parseInt(this.scrollHeight.slice(this.scrollHeight.indexOf('(') + 1, this.scrollHeight.indexOf('%')));
          const diffValue = parseInt(this.scrollHeight.slice(this.scrollHeight.indexOf('-') + 1, this.scrollHeight.indexOf(')')));
  ...
  openCell() {
    ...
    this.zone.runOutsideAngular(() => {
      setTimeout(() => {
        ...
        const focusableElement = DomHandler.findSingle(this.el.nativeElement, focusCellSelector);
...
export class TableHeaderCheckbox {
  ...
  isAllFilteredValuesChecked() {
    if (!this.dt.filteredValue) {
      ...
      for (const rowData of this.dt.filteredValue) {
