  ...
  implements ControlValueAccessor, Validator, OnInit, OnDestroy
{
  ...
  get isComplexValue() {
    return this.value && this.value.type && this.value.type.includes(".");
  }
  get isTypeControl() {
    return this.type && this.type.startsWith("Types.");
  }
  get isComplexControl() {
    return this.type && this.type.includes(".");
  }
  get isTypeValue() {
    return (
      this.value && this.value.type && this.value.type.startsWith("Types.")
    );
  }
  get EMPTY() {
  ...
  get isCatalogParent() {
    return this.type.startsWith("Catalog.") && this.id === "parent";
  }
  get isDocumentParent() {
    return (
      this.id === "parent" &&
      (Type.isDocument(this.type) || this.type === "Types.Document")
    );
  }
  ...
  ngOnInit() {
    ...
    this._docSubscription$ = this.ds.showDialog$
      .pipe(filter((uuid) => this.uuid === uuid.uuid))
      .subscribe((uuid) => {
  ...
  getSuggests(text: string) {
    ...
    this.Suggests$ = this.api.getSuggests(
  ...
  select = () =>
    setTimeout(() => {
      if (
        this.input &&
        this.input.inputEL &&
        this.input.inputEL.nativeElement
      ) {
  ...
  onArchivedClick(data, event) {
    event.stopPropagation();
    event.preventDefault();
    // this.ds.openSnackBar(
    //   "error",
    //   "Element is archived",
    //   `Selected element "${data.value}" and cant be used`
    // );
  ...
  searchComplete(row: ISuggest) {
    ...
    this.value = {
      id: this.isTypeValue ? null : row.id,
      code: row.code,
      type: row.type,
      value: this.isTypeValue ? null : row.value,
    };
  }
    ...
    if (this.owner && this.owner.length) {
      for (const row of this.owner) {
        ...
        if (row.dependsOn.length === 36) rightValue = row.dependsOn; // is GUID
          ...
          const fc =
            this.formControl.parent.get(row.dependsOn) ||
            this.formControl.root.get(row.dependsOn);
        ...
        if (rightValue)
          result.filter.push({
            ...
            center: "=",
            right: rightValue,
            isFixed: row.isOwnerFixed === true,
          });
      }
    }
    if (!this.useHierarchyList || suggest) {
      if (this.storageType === "folders") {
        result.filter.push({ left: "isfolder", center: "=", right: true });
      }
      if (this.storageType === "elements") {
        result.filter.push({ left: "isfolder", center: "=", right: false });
      }
      if (this.storageType === "all") {
        result.filter.push({ left: "isfolder", center: "=", right: undefined });
      }
    }
    if (
      !result.filter.find((e) => e.left === "company") &&
      !this.isDocumentParent
    ) {
      let company;
      if (Type.isDocument(this.type)) {
        ...
        if (doc && doc.company && doc.company.value && doc.company.value.id)
          company = doc.company.value.id;
      }
      if (company)
        result.filter.push({ left: "company", center: "=", right: company });
    }
  ...
  parseDate(dateString: string) {
    ...
    else if (!date && this.required)
      this.formControl.setErrors({ "invalid date": true });
