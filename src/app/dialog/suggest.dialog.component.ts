...
import { v1 } from 'uuid';
import { StorageType, FormListSettings, ISuggest, Type, ColumnDef, DocumentBase, DocumentOptions, FormListFilter, FormListOrder } from 'jetti-middle/dist';
@Component({
...
export class SuggestDialogComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(BaseTreeListComponent, { static: false })
  tl: BaseTreeListComponent;
  ...
  get sid() {
    return {
      id: this.selection && this.selection.length ? this.selection[0].id : "",
      type: this.type,
    };
  }
  set sid(value: { id: string; type?: string }) {
  ...
  async ngOnInit() {
    ...
    [...data, ...dimensions].forEach((el) => {
      ...
      columns.push({
        field,
        type: <string>((schema[field] && schema[field].type) || type),
        label: (schema[field] && schema[field].label) || field,
        hidden: !!(schema[field] && schema[field].hidden),
        required: true,
        readOnly: false,
        sort: new FormListOrder(field),
        order: (schema[field] && schema[field].order) || 0,
        style: (schema[field] && schema[field].style) || { width: "150px" },
        value: value,
        headerStyle: (schema[field] && schema[field].style) || {
          width: "150px",
          "text-align": "center",
        },
      });
    ...
    if (this.doc) {
      this.showTree =
        (this.doc.Prop as DocumentOptions).hierarchy === "folders";
    ...
    this._debonceSubscription$ = this.debonce$
